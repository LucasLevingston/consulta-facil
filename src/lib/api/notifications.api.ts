...
export const notificationsApi = {
  ...
  acceptInvite: async (id: string): Promise<NotificationResponse> => {
    const response = await api.put<NotificationResponse>(`/notifications/${id}/accept`);
    return response.data;
  },

  declineInvite: async (id: string): Promise<NotificationResponse> => {
    const response = await api.put<NotificationResponse>(`/notifications/${id}/decline`);
    return response.data;
  },

  sendClinicInvite: async (clinicId: string, doctorProfileId: string): Promise<void> => {
    await api.post(`/clinics/${clinicId}/invites/${doctorProfileId}`);
  },
};
