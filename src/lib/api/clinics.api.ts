...
import type { ClinicResponse, CreateClinicInput } from "@/lib/schemas/clinic.schema";
...
export const clinicsApi = {
  ...
  getNearby: async (lat: number, lng: number, radiusKm = 50): Promise<ClinicResponse[]> => {
  ...
  update: async (id: string, data: CreateClinicInput): Promise<ClinicResponse> => {
    const response = await api.put<ClinicResponse>(`/clinics/${id}`, data);
    return response.data;
  },

  addMember: async (clinicId: string, doctorProfileId: string): Promise<void> => {
    await api.post(`/clinics/${clinicId}/members/${doctorProfileId}`);
  },

  removeMember: async (clinicId: string, doctorProfileId: string): Promise<void> => {
    await api.delete(`/clinics/${clinicId}/members/${doctorProfileId}`);
  },
};
