...
const sizeMap = {
  sm: { avatar: "size-16", button: "h-6 w-6", icon: "h-3 w-3", text: "text-lg" },
  md: { avatar: "size-20", button: "h-7 w-7", icon: "h-3.5 w-3.5", text: "text-xl" },
  lg: { avatar: "size-24", button: "h-8 w-8", icon: "h-4 w-4", text: "text-2xl" },
};
...
export function AvatarUpload({ size = "md" }: AvatarUploadProps) {
  ...
  return (
    <div className="relative w-fit">
      <Avatar className={`${s.avatar} rounded-2xl border-4 border-card shadow-md`}>
        <AvatarImage src={user?.imageUrl ?? undefined} alt={user?.name ?? "Avatar"} />
        <AvatarFallback className={`rounded-2xl bg-primary/15 text-primary font-bold ${s.text}`}>
