...
import { createClinicSchema, type CreateClinicInput } from "@/lib/schemas/clinic.schema";
import { useMyClinic, useCreateClinic, useUpdateClinic } from "@/hooks/api/use-clinics";
...
function ClinicForm({ clinic }: { clinic?: ReturnType<typeof useMyClinic>["data"] extends (infer T)[] ? T : never }) {
  const isEdit = !!clinic;
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 max-w-2xl">
        <div className="space-y-4">
          ...
          <CustomFormField form={form} name="name" fieldType={FormFieldType.INPUT} />
          <CustomFormField form={form} name="description" fieldType={FormFieldType.TEXTAREA} />
          <CustomFormField form={form} name="phone" fieldType={FormFieldType.INPUT} />
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <CustomFormField form={form} name="address" fieldType={FormFieldType.INPUT} />
            <CustomFormField form={form} name="zipCode" fieldType={FormFieldType.INPUT} label="CEP" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <CustomFormField form={form} name="city" fieldType={FormFieldType.INPUT} label="Cidade" />
            <CustomFormField form={form} name="state" fieldType={FormFieldType.INPUT} label="Estado" />
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <LocationPicker lat={lat} lng={lng} onLocationSelect={handleLocationSelect} />
        </div>
