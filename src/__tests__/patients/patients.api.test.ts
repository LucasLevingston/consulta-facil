...
const patientSummaryPage = {
  content: [
    { id: "p-1", name: "Maria Silva", lastAppointment: "2026-05-10T10:00:00", totalAppointments: 3 },
  ],
  totalElements: 1,
  totalPages: 1,
  number: 0,
...
describe("patientsApi", () => {
  ...
  describe("updateMedicalRecords", () => {
    it("chama PUT /patients/:userId/medical-records e retorna o prontuário atualizado", async () => {
      ...
      const result = await patientsApi.updateMedicalRecords("u-1", { allergies: "Penicilina" });

      expect(mockPut).toHaveBeenCalledWith("/patients/u-1/medical-records", { allergies: "Penicilina" });
      expect(result.allergies).toBe("Penicilina");
    });
  });

  describe("getDoctorPatients", () => {
    it("chama GET /patients/doctor/:doctorId com os params e retorna a página", async () => {
      mockGet.mockResolvedValueOnce({ data: patientSummaryPage });

      const result = await patientsApi.getDoctorPatients("d-1", { page: 0, size: 20, search: "", sort: "recent" });

      expect(mockGet).toHaveBeenCalledWith("/patients/doctor/d-1", {
    ...
    it("passa parâmetros de busca e sort corretamente", async () => {
      ...
      await patientsApi.getDoctorPatients("d-1", { page: 1, size: 10, search: "Maria", sort: "name" });
