...
describe("examRequestApi — create", () => {
	...
	it("chama POST no appointmentId correto com payload", async () => {
		...
		const input = { examName: "Raio-X", instructions: "Em jejum" };

		await examRequestApi.create("apt-1", input);

		expect(mockPost).toHaveBeenCalledWith("/appointments/apt-1/exams", input);
	});
...
describe("examRequestApi — review avaliação pelo profissional", () => {
	beforeEach(() => vi.clearAllMocks());
	...
	it("chama PUT no examId correto com payload", async () => {
		...
		expect(result.professionalNotes).toBe("Resultado normal");
	});
